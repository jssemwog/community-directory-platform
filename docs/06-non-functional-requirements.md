...
| NFR-PRIV-04 | The system shall collect through the public form only the information needed for a listing, and shall not require personal information beyond the defined listing fields. | Privacy (data minimisation) | Scope: data; L2 | Should | Inspection | Data-minimisation principle. **Decided: OQ-8** — the public form requires only business name, category, description, locality, and country; administrative area, postal code, phone, email, and website are optional at initial submission. The before-approval contact minimum (FR-DATA-08, OQ-8b) requires **one** of phone, email, or website — a business contact method, not personal information, and not demanded before a request may enter moderation. No field is added by this decision. |
| NFR-PRIV-05 | Any retention of rejected submissions or other non-public data shall be limited to a defined, documented purpose and period rather than kept indefinitely by default. | Privacy | A5; FR-AUD-06 | **Must** | Inspection / Audit | **Decided: OQ-13 (2026-08-04). Raised from `Should` by OQ-13.** For rejected records the period is **90 days from the rejection**, and the documented purpose is **to allow an administrator to review, explain, or reconsider a moderation decision, and to provide moderation context, for a bounded time after rejection**. The same uniform rule covers rejected initial submissions and rejected approved-listing revisions (`FR-AUD-06`). **Data minimisation follows from the bound, not from goodwill:** retention that outlives the period is a defect, which is why `FR-AUD-06` makes purge an obligation rather than an option. This row does **not** set a period for any other non-public data — notably **unpublished approved listings, which OQ-13 explicitly excluded**, audit entries (`OQ-14`/`NOQ-8`, retention `NOQ-7`), and safeguard artifacts (`OQ-9`, `NFR-PRIV-04`). **Approved revision proposals** are not retained once their content is applied: they are removed in the same atomic unit that applies them (ungated Product Owner policy clarification, ADR-017 Q-3, issue #133), so **no retention purpose or period is set for them**. This does not decide general history or audit retention, which remain `OQ-14`/`NOQ-8` and `NOQ-7`. **The rejection period is measured from a write-once rejection timestamp** on the rejected listing or rejected revision, separate from last-updated, whose **only** purpose is retention eligibility; it is administrator-visible, never public, and is purged with its record (ungated Product Owner policy clarification, ADR-017 Q-1/Q-2, issue #137). |
...
| NFR-DATA-05 | The submission date shall be recorded once at submission and not change, and the last-updated date shall change whenever a record's content or status changes. | Data integrity | FR-AUD-02, FR-AUD-03 | Must | Test | Accurate temporal metadata. **Timestamp semantics (ungated Product Owner ruling, issue #141):** submission and last-updated are **initialized to the same caller-supplied instant** at submission; every successful content change and every permitted status transition requires an instant **strictly later** than the current last-updated, and an **equal or earlier** one is **refused without mutation** — an equal instant would not change the value this row requires to change. **Publication-state changes move no timestamp** (publication is separate from content and status — `OQ-11`, `ADR-006`), and **refused actions change none**. Time is **supplied by the caller**; no ambient clock is read. Logical semantics only — the physical representation remains `ADR-017`'s. |
...
**Backup and recovery — decided (`NOQ-3`, 2026-07-30; amended R1, 2026-08-18).** The
committed MVP posture is: **daily backups**; a **recovery point objective (RPO — the maximum
amount of recent data it is acceptable to lose in a failure) of up to 24 hours**; a **recovery
time objective (RTO — the maximum acceptable time to restore service) of one business day**; a
**restore test before launch and at least quarterly thereafter**; and **at least one
independent backup copy kept outside the primary provider, no more than 24 hours behind the
live data**. **Every copy of the data — live data, provider-managed backups, exported backups,
and independent off-provider copies — carries the same confidentiality, access-control, and
protection guarantees.** **Amended (R1, 2026-08-18):** the 24-hour recovery point objective
applies to **every in-scope recovery scenario**, including complete loss of — or loss of access
to — the primary provider; the original *at least weekly* off-provider refresh guarantee is
**superseded**, because a weekly minimum permits a copy up to seven days old. This posture is
...
| NFR-BACK-03 | Recovery shall meet defined objectives for maximum tolerable data loss (**recovery point objective, RPO**) and maximum tolerable downtime (**recovery time objective, RTO**): an **RPO of up to 24 hours** and an **RTO of one business day**. | Backup and recovery | Stakeholders | Should | Demonstration / Analysis | **Decided (`NOQ-3`, 2026-07-30): RPO up to 24 hours; RTO one business day** — restoration completed within one normal working day of a maintainer detecting the loss. **Scope (R1, 2026-08-18):** this recovery point objective governs **every in-scope recovery scenario** — ordinary recovery, **complete loss of (or loss of access to) the primary provider**, accidental deletion or operator error, and data corruption. **There is no weaker provider-loss objective**, and the objective values are unchanged. |
| NFR-BACK-04 | Every copy of the data — **live data, provider-managed backups, exported backups, and independent off-provider copies** — shall carry **equivalent confidentiality, access control, and protection**, including exclusion of unauthorized access to non-public submission data. | Backup and recovery / Privacy | Privacy; NFR-PRIV-03 | Must | Inspection | Backups are a data-exposure surface too. **Decided (`NOQ-3`, 2026-07-30): applies to all copies, including the independent off-provider copy (`NFR-BACK-06`).** **Purge and backups — decided by OQ-13 (2026-08-04), stated as narrowly as the question allows:** the `FR-AUD-06` purge obligation applies to the **live product** — a purged record leaves operational use and administrative visibility; **copies of a purged record may temporarily persist in backup or disaster-recovery media** taken before the purge, and remain subject to this row's equivalent-protection requirement while they do; and a **restoration shall not silently return an expired or purged rejected record to live product use**. Backup retention, rotation, deletion mechanics, and restoration procedure remain **deferred** — no schedule, product, or mechanism is selected here. |
...
| NFR-BACK-06 | At least one backup copy shall be kept in a location or account **independent of the primary data-store provider**, and shall be **no more than 24 hours behind the committed state of the live data**, so that the data survives the loss of — or loss of access to — the primary provider **without exceeding the recovery point objective in `NFR-BACK-03`**. | Backup and recovery | Stakeholders (maintainers); `NOQ-3` | Must | Inspection / Demonstration | **Decided (`NOQ-3`, 2026-07-30); amended (R1, 2026-08-18).** The original *refreshed at least weekly* guarantee is **superseded**: a weekly minimum permits a copy up to seven days old and cannot satisfy the 24-hour recovery point objective under provider loss. The requirement now constrains **staleness, not cadence** — a refresh cadence alone is insufficient, because backup generation and copy transfer lags **compound**. Provider-neutral: an off-provider copy, not a named product or service; **no mechanism is selected**. The same confidentiality and access-control guarantees apply (`NFR-BACK-04`). **This is a freshness obligation only** — it does not require the independent copy to preserve any finer-grained recovery capability. |
...
| NOQ-3 | ~~What are the committed backup frequency, recovery point (max data loss), and recovery time (max downtime)?~~ **Decided (2026-07-30):** daily backups; RPO up to 24 h; RTO one business day; restore-tested before launch and quarterly; ~~≥ weekly independent off-provider copy~~ **independent off-provider copy no more than 24 h behind the live data (amended R1, 2026-08-18)**; equivalent confidentiality for all copies. See `NFR-BACK-01`–`NFR-BACK-06` and `docs/13` `NOQ-3`. | NFR-BACK-01, NFR-BACK-03 | Stakeholders (maintainers) |
...
NFR-DATA-02; **Decided** 2026-08-04, and it adds **no status transition**), and `OQ-13`
(rejected-record retention and purge — NFR-PRIV-05; **Decided** 2026-08-04: 90 days from
rejection, then purge); `OQ-9`
(anti-spam safeguard — NFR-SEC-06), and `OQ-14` (audit logging — NFR-OBS-05) remain open in
...
| NFR-PRIV-04 | ~~OQ-8~~, ~~OQ-8b~~ — **Decided** |
| NFR-PRIV-05 | **No open question remains** — Decided (`OQ-13`): 90 days from rejection, with a documented purpose |
