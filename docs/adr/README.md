...
**It holds nine decisions in force.** `ADR-001`, `ADR-002`, `ADR-003`, `ADR-005`, `ADR-006`,
`ADR-010`, `ADR-013`, `ADR-012` — since 2026-09-04 — and **`ADR-014`, since 2026-09-11**, are all
**Accepted**; **`ADR-015` is `Proposed` — drafted, under review, and *not* in force** (issue #119),
so **the count of decisions in force remains nine**; and every remaining
...
| **[`ADR-015`](ADR-015-migration-and-schema-evolution-tooling.md)** | **Migration and schema-evolution tooling** — `DDM-10`, the decision `ADR-014` named as a **separately governed** successor work unit and deliberately did not create | *none* — **`DG-2`** is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** one of its constituents | **Nothing.** `DDM-10` carries **no blocker** in `docs/08`. **Fed by `ADR-003`** and **`ADR-013`** (the engine and the named provider), **`ADR-014`** (the accepted `C9` access approach, which classified Kysely as **`LEAVES OPEN`** for `DDM-10`), **`ADR-005`** (the runtime posture) and **`ADR-012`** (the runner). **Not named by `docs/07`** — a decision the chain did not anticipate, taking `ADR-015` under *Numbering and filenames* | **`Proposed`** — 2026-09-13 (issue #119). ***NOT in force; nothing may depend on it.*** **No owner ruling has occurred and `DDM-10` remains unresolved.** The document carries an **evidence-based recommendation for owner consideration only** — Kysely's built-in `Migrator`, argued principally because it **keeps `ADR-014`'s deferred driver/client decision free of earlier migration-side pressure**, whereas `node-pg-migrate` requires `pg` for the migration tool — not a runtime driver selection and not an architectural foreclosure (a two-client configuration remains possible), but a governance and implementation-convenience pressure toward `pg`; `node-pg-migrate` is preserved as a serious alternative that is **better on migration-history portability**, and the rejection rationale for every category is preserved (`IR-6`). **A recommendation is not an acceptance**: the **two-stage `Proposed` → `Accepted` lifecycle applies**, a mandatory detailed owner review precedes acceptance, and the acceptance work unit is **not created**. **It selects migration/schema-evolution tooling only** — and even on acceptance would select **no** PostgreSQL driver/client, **no** Kysely dialect, **no** pooler, **no** migration authoring format beyond what the candidate makes inseparable, **no** migration execution environment or timing, **no** rollback policy, and **no** physical schema, table, column, key, constraint or index (**`DDM-2`–`DDM-9` remain unresolved**). **Nothing is installed, no dependency is added, and no implementation, provisioning, CI or `DG-4` change is authorized** |
