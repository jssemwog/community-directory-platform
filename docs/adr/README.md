...
**It holds twelve decisions in force.** `ADR-001`, `ADR-002`, `ADR-003`, `ADR-005`, `ADR-006`,
`ADR-010`, `ADR-013`, `ADR-012` — since 2026-09-04 — `ADR-014` — since 2026-09-11 — `ADR-015` —
since 2026-09-14 — `ADR-016` — since 2026-09-15 — and **`ADR-017`, since 2026-09-17** (issue
#137; published as `Proposed` 2026-09-16, issue #131), are all **Accepted**; and every remaining
...
| **[`ADR-014`](ADR-014-postgresql-data-access-approach.md)** | **`C9`'s PostgreSQL data-access approach** — the ORM / query-builder / direct-driver question owner ruling **`R-A`** deferred; **the driver/client itself stays deferred** | *none* — **`DG-2`** is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** one of its constituents | **Nothing.** `ADR-002` (Accepted) established **ruling `R-A`**, deferring every ORM, data-access, query-builder, client and migration choice, and `ADR-005`, `ADR-012` and `ADR-013` restated it; `ADR-012` recorded persistence technology as *"a **separate later work unit**, approved in principle only"*. **Fed by `ADR-003`** and **`ADR-013`** (the engine and the named provider), **`ADR-005`** (the long-running Node runtime), **`ADR-006`** (the lifecycle the store must express), **`ADR-010`** (the recovery-correctness criteria) and **`ADR-012`** (the runner). **Not named by `docs/07`** — a decision the chain did not anticipate, taking `ADR-014` under *Numbering and filenames* | **Accepted** — 2026-09-11 (issue #117). *In force; later `C9` implementation may rely on it.* Published as `Proposed` 2026-09-09 (issue #115, PR #116). The decision is a **typed SQL query-builder approach — Kysely** — as the `C9` PostgreSQL data-access approach, the product owner's selection from the governed candidate comparison; explicit transaction control is part of the selected approach. **The PostgreSQL driver/client is deliberately NOT selected** and remains an implementation-time deferred choice, as does the **connection pooler**. **`DDM-10` is NOT resolved** and remains a **separately governed** later work unit, which is **not created** by this ADR — **no candidate forecloses it**; the comparison classifies Prisma and Drizzle as **narrowing** it and Kysely and direct `pg` as **leaving it open**. **`DDM-2`–`DDM-9` remained unresolved at this ADR's acceptance** (`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` since discharged by `ADR-017`, 2026-09-17), and this ADR expressly does **not** claim that selecting any candidate would resolve them; and no driver, physical schema, index, constraint, migration, pooler, pool size, PostgreSQL version, provisioning, credential, database-test infrastructure, coverage, CI or `DG-4` decision is made. **Acceptance is not installation**: Kysely is the `Accepted` approach decision, not an installed package, and **no** dependency installation and **no** `C9` implementation is authorized — each remains a separate later work unit |
| **[`ADR-015`](ADR-015-migration-and-schema-evolution-tooling.md)** | **Migration and schema-evolution tooling** — `DDM-10`, the decision `ADR-014` named as a **separately governed** successor work unit and deliberately did not create | *none* — **`DG-2`** is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** one of its constituents | **Nothing.** `DDM-10` carries **no blocker** in `docs/08`. **Fed by `ADR-003`** and **`ADR-013`** (the engine and the named provider), **`ADR-014`** (the accepted `C9` access approach, which classified Kysely as **`LEAVES OPEN`** for `DDM-10`), **`ADR-005`** (the runtime posture) and **`ADR-012`** (the runner). **Not named by `docs/07`** — a decision the chain did not anticipate, taking `ADR-015` under *Numbering and filenames* | **Accepted** — 2026-09-14 (issue #121). *In force; work may rely on it.* Published as `Proposed` 2026-09-13 (issue #119, PR #120). The decision is **Kysely Migrator** — Kysely's built-in `Migrator` — as the migration and schema-evolution tooling approach, the product owner's selection from the governed candidate comparison, on the owner's stated grounds — including that it **keeps `ADR-014`'s deferred driver/client decision free of earlier migration-side pressure**, whereas `node-pg-migrate` requires `pg` for the migration tool — not a runtime driver selection and not an architectural foreclosure (a two-client configuration remains possible), but a governance and implementation-convenience pressure toward `pg`; `node-pg-migrate` is preserved as a serious alternative that is **better on migration-history portability**, and the rejection rationale for every category is preserved (`IR-6`). **`DDM-10` is resolved by this `Accepted` ADR.** **It selects migration/schema-evolution tooling only** — and selects **no** PostgreSQL driver/client, **no** Kysely dialect, **no** pooler, **no** migration authoring format beyond what the candidate makes inseparable, **no** migration execution environment or timing, **no** rollback policy, and **no** physical schema, table, column, key, constraint or index (**`DDM-2`–`DDM-9` remained unresolved at this ADR's acceptance** (`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` since discharged by `ADR-017`, 2026-09-17)). **Nothing is installed, no dependency is added, and no implementation, provisioning, CI or `DG-4` change is authorized** |
| **[`ADR-016`](ADR-016-postgresql-runtime-driver-and-kysely-dialect.md)** | **Runtime PostgreSQL driver/client and its Kysely dialect for `C9`** — the choice `ADR-014` deliberately deferred; a **non-DDM** technology-selection decision | *none* — **`DG-2`** is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** one of its constituents | **Nothing.** **Fed by `ADR-003`**, **`ADR-005`** (the long-running Node runtime), **`ADR-013`** (the named provider), **`ADR-014`** (Kysely, which deferred the driver) and **`ADR-015`** (Kysely `Migrator`). **Not named by `docs/07`** — a decision the chain did not anticipate, taking `ADR-016` under *Numbering and filenames* | **Accepted** — 2026-09-15 (issue #125). *In force; work may rely on it.* Published as `Proposed` 2026-09-14 (issue #123, PR #124). Decision question: *which PostgreSQL client library, through which Kysely dialect, will `C9` and Kysely `Migrator` use to connect at runtime?* — the driver is the primary choice and the dialect is named as its inseparable consequence. The decision is **`pg` (node-postgres) through Kysely core `PostgresDialect`**, the Product Owner's approval on 2026-09-15 of Option A from the governed comparison with Postgres.js through `kysely-postgres-js`, which is preserved as a legitimate alternative (`IR-6`). **No numbered `DDM` owns or is resolved by this ADR.** It selects **no** package version, pool configuration, external pooler, TLS policy, secrets, provisioning, migration execution or rollback policy, and **no** physical schema (**`DDM-2`–`DDM-9` remained unresolved at this ADR's acceptance** (`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` since discharged by `ADR-017`, 2026-09-17)). **Nothing is installed and no implementation is authorized** |
| **[`ADR-017`](ADR-017-physical-listing-data-design.md)** | **Physical listing data design** — `DDM-2` identity strategy, `DDM-6` non-public attributes and per-value display designations, `DDM-8` revision storage and the effective public version, and `DDM-9` publication state, deletion and retention/purge, proposed as one coordinated design; coordinating `DDM-8` and `DDM-9` is a **design recommendation**, not a mandatory sequencing rule | *none* — **`DG-1`** (2026-08-04) and **`DG-2`** (2026-08-27, issue #93) are **`Resolved`** and this ADR is a constituent of neither | **Nothing.** `DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` carry no unresolved blocker in `docs/08`. **Must conform to `ADR-006`** (Accepted) and be consistent with **`ADR-014`**, **`ADR-015`** and **`ADR-016`** (all Accepted). **Not named by `docs/07`** — a decision the chain did not anticipate, taking `ADR-017` under *Numbering and filenames* | **Accepted** — 2026-09-17 (issue #137). *In force; work may rely on it.* Published as `Proposed` 2026-09-16 (issue #131). The Product Owner approved physical selections **PS-1 through PS-11** and their documented deferrals: opaque random UUID identity; non-public attributes on the same row with mandatory per-value designations defaulting to *not public* and one public read projection; effective content on the listing row with separate complete-content revision rows applied in one transaction; a publication-state datum constrained to approved listings, no deletion path for approved listings, derived purge-eligibility and physical purge of rejected records; listing status as a checked three-value datum; approved revision rows removed in the applying transaction, only *pending* and *rejected* revision states persisted; write-once rejection timestamps; and a per-listing row lock serializing revision work, with pending uniqueness as a backstop. The **five Product Owner questions** it raised are **ruled** — Q-3 (issue #133), Q-4 (issue #135), Q-1, Q-2 and Q-5 (issue #137). **`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` are discharged by this `Accepted` ADR.** **Deferred with the selections:** stale-edit detection and resolution policy, replacement-value semantics, restriction-only enforcement, isolation level, identity generation locus, projection form, and purge scheduling. **No later-publication route exists.** **Excluded:** category representation (`DDM-3`; **category persistence requires `OQ-5`/`DDM-3` first**), `DDM-4`, `DDM-5`, `DDM-7`, `OQ-4`, `OQ-5`, `OQ-14`, seam `S-7`, migration execution and rollback policy, provisioning, pool/TLS/secrets/environment configuration, integration-test infrastructure and CI. **This ADR introduces no executable SQL, schema files, migrations or application code; this ADR installs nothing** (`pg` 8.23.0 and `kysely` 0.29.6 were already installed through issue #129, PR #130; `@types/pg` remains absent and deferred)**; and no implementation is authorized** |
...
lifecycle states** — and **does not resolve `DDM-8` or `DDM-9`**. **`DDM-8` is
responsible for the physical revision-storage representation and for how the effective public
version is carried; `DDM-9` is responsible for the physical lifecycle and removal
representation — publication state, retention, purge, and any deletion semantics.** Both
remained **unresolved** until `ADR-017` was `Accepted` (2026-09-17, issue #137), which selects them. `ADR-006` states the *logical requirements* their later representations
