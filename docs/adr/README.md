...
| **[`ADR-002`](ADR-002-application-language-and-framework.md)** | Application language and framework | **`DG-2`** | **Nothing.** ~~The technology decision~~ — **no open question is named**; hard requirements 1, 2, 6 and `DD-2` are stated **inputs**, and `NOQ-1`/`NOQ-4` are **shaping inputs** that do not block (carried as `PA-1`). **Fed by `ADR-003`** (Accepted), which fixes the store the stack must integrate with | **Proposed** — 2026-08-24 (issue #85). *Drafted and **under review**; **not in force**, and nothing may depend on it.* The proposal is **TypeScript + Next.js**, the product owner's selection from the governed candidate comparison. **`DD-2` is not discharged**; `ADR-005` remains open; **`DG-2` remains Unresolved**; and no ORM/data-access technology, hosting provider, authentication mechanism, test tooling, CI, named PostgreSQL provider, schema, or separable frontend technology is selected. |
...
| **`ADR-012`** | Testing strategy — with the administrative boundary as its highest-value target | **`DG-2`** (tooling) · **`DG-4`** (depth) | `docs/07` names **no blocking question of its own** — but the **test tooling follows `ADR-002`**, which is **`Proposed`** and therefore not yet in force, and testing **depth** is `DG-4`. **It cannot be written in full until `ADR-002` lands** | **Not yet writable** |
