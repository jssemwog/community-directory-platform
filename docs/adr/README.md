...
**It holds one decision in force, and one under review.** `ADR-001` is **Accepted**;
`ADR-006` is **Proposed** and **not yet in force**; every other ADR is **blocked** on a
...
| **[`ADR-006`](ADR-006-listing-data-model-and-lifecycle-states.md)** | **Listing data model and lifecycle states** | **`DG-1`** — **Resolved 2026-08-04** | **Nothing.** `OQ-6`, `OQ-7`, `OQ-8`/`OQ-8b`, `OQ-10`, `OQ-11`, and `OQ-13` are all **Decided** — `DD-1` | **Proposed** — 2026-08-07 (issue #61). *Not in force; nothing may depend on it.* |
...
**`ADR-006` is now `Proposed`, and the distinction matters.** It began blocked on six
...
**Resolved**, the gate that held this ADR is open, and the ADR has now been **drafted** and
is **under review**. **That is not acceptance.** A `Proposed` decision is **not in force**;
**no work may depend on it** until it is **Accepted** through the normal review and merge
decision.
...
must satisfy; **once `ADR-006` is Accepted, those decisions must conform to it**, and until
