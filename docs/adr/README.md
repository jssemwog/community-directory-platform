...
**It holds ten decisions in force.** `ADR-001`, `ADR-002`, `ADR-003`, `ADR-005`, `ADR-006`,
`ADR-010`, `ADR-013`, `ADR-012` — since 2026-09-04 — `ADR-014` — since 2026-09-11 — and
**`ADR-015`, since 2026-09-14**, are all **Accepted**; **`ADR-016` is `Proposed` — drafted, under
review, and *not* in force** (issue #123), so **the count of decisions in force remains ten**; and
every remaining
...
| **[`ADR-016`](ADR-016-postgresql-runtime-driver-and-kysely-dialect.md)** | **Runtime PostgreSQL driver/client and its Kysely dialect for `C9`** — the choice `ADR-014` deliberately deferred; a **non-DDM** technology-selection decision | *none* — **`DG-2`** is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** one of its constituents | **Nothing.** **Fed by `ADR-003`**, **`ADR-005`** (the long-running Node runtime), **`ADR-013`** (the named provider), **`ADR-014`** (Kysely, which deferred the driver) and **`ADR-015`** (Kysely `Migrator`). **Not named by `docs/07`** — a decision the chain did not anticipate, taking `ADR-016` under *Numbering and filenames* | **`Proposed`** — 2026-09-14 (issue #123). ***NOT in force; nothing may depend on it.*** **Product Owner ruling recorded 2026-09-15.** Decision question: *which PostgreSQL client library, through which Kysely dialect, will `C9` and Kysely `Migrator` use to connect at runtime?* — the driver is the primary choice and the dialect is named as its inseparable consequence. The document compares `pg` through Kysely core `PostgresDialect` with Postgres.js through `kysely-postgres-js`, and carries a recommendation **for Product Owner consideration only** — `pg` through `PostgresDialect` — with Postgres.js preserved as a legitimate alternative (`IR-6`). **Owner decision: Option A approved 2026-09-15** — `pg` (node-postgres) through Kysely core `PostgresDialect`; **decided, but not yet in force** until the Proposal and Acceptance lifecycles complete. **No numbered `DDM` owns or is resolved by this ADR.** Even on acceptance it would select **no** package version, pool configuration, external pooler, TLS policy, secrets, provisioning, migration execution or rollback policy, and **no** physical schema (**`DDM-2`–`DDM-9` remain unresolved**). **Nothing is installed and no implementation is authorized** |
