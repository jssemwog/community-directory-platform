...
- **Server rendering by default.** Layouts and pages are Server Components by default, which keeps the public surface server-rendered as hard requirement 1 requires, and form submissions via Server Actions support progressive enhancement.
...
- **No first-party request-security default comparable to the alternatives.** All five alternatives ship an explicit, developer-visible CSRF mechanism enabled by default. Request security here must be addressed deliberately.
...
**Reversibility:** **Expensive, but bounded, and it degrades gracefully.** Reversing the language and framework means rewriting the delivery layer — routes, rendering, and request handling — and is the most expensive reversal available short of reopening `ADR-001`. What makes it bounded is exactly the discipline this ADR requires: if domain and application logic are kept in plain TypeScript modules behind the `C1`–`C12` seams, with `C9` the sole data-access path, then the domain survives a delivery-layer replacement and the data model is untouched. **If that discipline lapses, reversibility lapses with it.** This is the single strongest practical argument for treating the obligations below as binding.
...
| **O-6** | **Request security is deliberately reviewed and addressed** for every state-changing entry point, since no developer-visible framework default is being relied upon. |
...
| **Request-security handling is uneven**, because no developer-visible framework default is being relied upon | A state-changing action is exploitable cross-origin | **O-6** — deliberate review of every state-changing entry point, recorded rather than assumed |
...
| Next.js — Server and Client Components; Forms; Self-hosting (official docs) | Server Components by default; the `'use client'` module-graph boundary; server-only environment variables and the server-only import guard; progressive enhancement for form actions; self-hosting as a Node.js server, Docker image, or static export; per-instance cache and multi-instance coordination requirements |
