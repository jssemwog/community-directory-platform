...
> **Proposed, and therefore not in force.** Per `docs/adr/README.md`, a `Proposed` ADR is *"Drafted and under review. The decision is **not yet in force**; nothing may depend on it."* This document records the product owner's test-tooling selection of 2026-09-03 for review; **acceptance is a separate governed step**, following the `ADR-002` / `ADR-003` / `ADR-005` / `ADR-013` precedent of a `Proposed` publication (issue #101 / PR #102) followed by a separate owner `Accepted` ruling (issue #103 / PR #104). **Publication authorizes nothing**: no dependency is installed, no `package.json` or `package-lock.json` change is made, no test script, configuration, or test file is created, no CI change is made, and **`P1` Slice A is not authorized to begin**.
...
|---|---|---|
| **`Proposed` is treated as in force** | A dependency is installed, or Slice A begins, on the strength of a draft | The header callout states that this ADR is **not in force**, that acceptance is a **separate governed step**, and that publication authorizes no installation and no implementation |
...
| **The proposal is read as authorizing `P1` Slice A** | Implementation begins without its own commissioned work unit | This ADR authorizes no implementation; Slice A remains a **separate later work unit**, not commissioned by this document |
...
**This document is `Proposed` only.** Under `docs/adr/README.md`'s statuses, a `Proposed` ADR is **not in force** and **nothing may depend on it**; only an `Accepted` ADR is *"Decided and in force. Work may rely on it."* **A separate later owner acceptance is required**, as its own issue and pull request, following the uniform `ADR-002` / `ADR-003` / `ADR-005` / `ADR-013` precedent. **Publication itself authorizes no dependency installation, no configuration, no test, no CI change and no implementation.**
...
| **Implementation** | **Not authorized.** This ADR is `Proposed`; installation and `P1` Slice A are separate later work units |
...
| **Decisions** | **`ADR-012`'s test-tooling portion** — proposed here, and **not in force while this ADR is `Proposed`**. **`ADR-012`'s depth portion — untouched**, under `DG-4`. `DD-2` — already discharged by `ADR-003`'s sibling `ADR-002`; untouched. `DG-2` — **`Resolved`** 2026-08-27 (issue #93); this ADR is not a constituent and changes nothing about it. `DG-4` — **`Unresolved`**; unchanged |
...
| **Documents amended** | **At this `Proposed` stage:** this file, `docs/adr/README.md` (register row, the register's `Blocked`/`Not yet writable` framing, and the derived cannot-be-written count) and `docs/traceability-matrix.md` (ADR register row), plus `docs/13-decision-log.md`, whose statement that `ADR-012` *"remains unwritten"* this publication makes false. **`docs/11-test-strategy.md` and `docs/12-implementation-plan.md` are untouched**: a `Proposed` ADR puts nothing in force, so `docs/11`'s *"No automation tool, runner, or framework is selected"* and `docs/12`'s *"the testing tool still follows from it via `ADR-012`"* both remain true. **At acceptance:** the `Accepted` lifecycle status, and whichever of those statements acceptance then makes false — determined by that separate work unit, not pre-empted here |
| **Issue / pull request** | **Proposed:** issue #105 — `architecture: decide the MVP test tooling in ADR-012`. **Accepted:** a separate governed step, not yet opened |
