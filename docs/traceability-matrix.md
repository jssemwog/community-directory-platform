...
| [`ADR-012`](adr/ADR-012-test-tooling.md) | Testing strategy — **tooling half** | **Proposed** — 2026-09-03 (issue #105); **not in force**. The proposed decision is **Vitest** as the primary implementation-level test runner, beginning with **`P1` Slice A**, whose logical/domain foundation is its first consumer | **Nothing, for the tooling half** — `ADR-002` (**Accepted** 2026-08-25) fixes the stack it must serve. Testing **depth** remains **`DG-4`**, which is **`Unresolved`** and **not** resolved by selecting a runner: no coverage provider, coverage threshold or release-readiness criterion is decided (`NFR-MAINT-03`, `R-6`). **`DG-2` remains `Resolved`** (2026-08-27, issue #93) and is **not** reopened; `ADR-012` is **not** a `DG-2` constituent. **Not yet Accepted or in force** — acceptance is a separate governed step. No version, installation, script, configuration or complementary-layer tooling is selected; **Slice A decides no `DDM-*`**; and no persistence technology or provisioning decision arises |
