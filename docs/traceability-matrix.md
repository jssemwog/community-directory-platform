...
| [`ADR-016`](adr/ADR-016-postgresql-runtime-driver-and-kysely-dialect.md) | Runtime PostgreSQL driver/client and Kysely dialect for `C9` (non-DDM) | **`Proposed`** — 2026-09-14 (issue **#123**); ***NOT accepted, NOT in force, and nothing may depend on it***. **No pull request yet; no acceptance issue.** The document compares `pg` through Kysely core `PostgresDialect` with Postgres.js through `kysely-postgres-js` and carries a recommendation **for Product Owner consideration only** — `pg` through `PostgresDialect` — which is **explicitly non-authoritative**; Postgres.js is preserved as a legitimate alternative (`IR-6`). **Owner decision: Option A approved 2026-09-15** — `pg` (node-postgres) through Kysely core `PostgresDialect`; **decided, but not yet in force** | **Nothing** — [`ADR-003`](adr/ADR-003-data-store-product.md), [`ADR-005`](adr/ADR-005-hosting-platform-and-runtime-model.md), [`ADR-013`](adr/ADR-013-managed-postgresql-provider.md), [`ADR-014`](adr/ADR-014-postgresql-data-access-approach.md) and [`ADR-015`](adr/ADR-015-migration-and-schema-evolution-tooling.md) (all **Accepted**) supply every constraint. **No numbered `DDM` owns or is resolved by this decision**; `DDM-2`–`DDM-9` remain unresolved and `DDM-10` (resolved by `ADR-015`) is unaffected. **The Product Owner's choice of driver/client and Kysely dialect is recorded but not yet in force (ADR-016 is not `Accepted`)**, and ADR-016 selects nothing further: unselected are **application pool configuration** and any **external pooler**; no package version, TLS policy, secrets, provisioning, migration execution or rollback decision is made. **No package is installed and no implementation exists or is authorized**; the register's decisions-in-force count **remains ten** |
