...
| [`ADR-015`](adr/ADR-015-migration-and-schema-evolution-tooling.md) | Migration and schema-evolution tooling (`DDM-10`) | **`Proposed`** — 2026-09-13 (issue **#119**); ***NOT accepted, NOT in force, and nothing may depend on it***. **No pull request yet; no acceptance issue; no acceptance date; no owner ruling.** The document carries an **evidence-based recommendation for owner consideration only** — Kysely's built-in `Migrator` — which is **explicitly non-authoritative**; `node-pg-migrate` is preserved as a serious alternative, stronger on migration-history portability, and rejection rationale is preserved for every category (`IR-6`). **Owner decision: PENDING** | **Nothing** — `DDM-10` carries **no blocker** in [`docs/08`](08-data-model.md); [`ADR-003`](adr/ADR-003-data-store-product.md), [`ADR-013`](adr/ADR-013-managed-postgresql-provider.md) and [`ADR-014`](adr/ADR-014-postgresql-data-access-approach.md) (all **Accepted**) supply the engine, the named provider and the `C9` access approach, and `ADR-014` classified Kysely as **`LEAVES OPEN`** for `DDM-10`; `DG-2` is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** a constituent of it. **`DDM-10` is NOT resolved** — it would be resolved only on acceptance, and [`docs/08`](08-data-model.md)'s `DDM-10` row is **left unamended**. **The PostgreSQL driver/client remains deferred and unselected** (neither `pg` nor Postgres.js nor any other), as do the **Kysely dialect**, the **application connection pool** and any **external pooler**. **`DDM-2`–`DDM-9` remain unresolved**, and **migration contents, migration authoring format beyond what the candidate makes inseparable, migration execution environment and timing, and rollback/down-migration policy all remain unresolved**; `ADR-010`'s **independent off-provider copy**, **restore rehearsals** and **provider-capability validation** remain **outstanding**, and `ADR-010` is expressly recorded as imposing **no** constraint on the candidate set. No physical schema, table, column, key, constraint, index, PostgreSQL version, provisioning, credential, secret, CI database infrastructure, coverage or `DG-4` decision is made. **Nothing is installed, no dependency is added, and no implementation exists or is authorized**; the register's decisions-in-force count **remains nine** |
