...
| `ADR-005` | Hosting platform & runtime model | **Ready to write** | **Nothing** — ~~`DG-2` (`DD-5`)~~; `DD-5` is a stated **input** and `NOQ-2` is **Decided**. **`DG-2` membership stands**, and `ADR-005` is `DG-2`'s **final remaining substantive constituent** (owner ruling, issue #87) |
