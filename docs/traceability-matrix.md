...
|---|---|---|---|---|---|
| **FR-VIS-01…10** (visitor browse/view) | V1, V5, V6, V7 | Public read · **P2** | `OQ-3` (ordering); ~~`OQ-7`~~ **Decided** (public fields), ~~`OQ-11`~~ **Decided** (`FR-VIS-08` trigger — *not currently publicly available, for any reason*; **generic** result, unpublished listings included) | **Public read path now serves the approved public projection** (`FR-DATA-11`/`11b`/`11c`, `docs/08` *Field classification*); **VIS-10 Deferred** (`OQ-1`); rest Committed | `FR-VIS-08` — **domain level, `P1` Slice B** (issue #139): the logical public projection returns **one** unavailable outcome for absent, *pending*, *rejected* and **unpublished** listings, disclosing nothing (`src/domain/listing/public-projection.ts`, `public-projection.test.ts`). The `P2` operation-level `BI-4` equivalence test remains its own issue |
...
| **FR-ADM-01…13** (administrator actions, incl. `FR-ADM-10b`) | A1–A7 | Administrative · **P4** | `OQ-12` (duplicates); ~~`OQ-10`~~, ~~`OQ-8`/`OQ-8b`~~, ~~`OQ-11`~~ **Decided** (`FR-ADM-12` — unpublish **and** republish Committed, **raised to Must**; a **repeated** unpublish or republish is **refused** and changes nothing — ungated clarification, issue #139, `docs/13`) | **Administrator completion Committed** (`OQ-8` — settles **C-7**): `FR-ADM-04` — an administrator may complete missing optional information and correct submitted information during moderation before approval, **validated identically to a public submission; never a bypass**. `FR-ADM-06` — approval permitted only where every before-approval obligation holds, **including the contact minimum**. **Revision lifecycle Committed** (`OQ-10`): `FR-ADM-10` **Must** — pending revision, approved listing stays public, approval makes the revision the effective public version, rejection leaves it unchanged; `FR-ADM-10b` **Must** — safeguarded atomic administrator operation, **refused while a revision is pending** (ungated Product Owner policy clarification, issue #137 — `OP-6`/`S7`). **ADM-12/13 Blocked/Conditional**; rest Committed | `FR-ADM-10` — **domain level, `P1` Slice D** (issue #145): the pending state's two exits are implemented and attacked in `src/domain/listing/revision.ts` and `revision.test.ts`. The proposal is named by `DI-11` — each operation resolves the listing's **sole** pending revision, so no logical revision identifier, ordering identity or equivalence rule is introduced here (`ADR-017` has discharged `DDM-8`'s design decision; implementing the selected physical revision representation is not part of Slice D), and nothing pending, more than one pending, a pending revision of another listing, a malformed collection and a malformed entry are each a distinct governed refusal. Approval applies the proposal's content and removes **that approved proposal** as one result — the listing keeping its identity, *approved* status, `submittedAt` and publication state, including staying **unpublished** — and rejection leaves the listing unchanged and writes the revision's **write-once rejection anchor**; either exit frees `DI-11`'s admission slot, and rejected and other retained revisions keep their place and value (`FR-AUD-06`). **Domain-level only** — `FR-ADM-10b`, `OP-10`/`OP-6`, `S7`/`S6` and journey `A6` are application and interface work for a later phase, not implemented here; store-level atomicity (`BI-7`, `C9`, `DDM-8`) is a persistence concern proven where it is enforced; retention eligibility, purge and audit remain outstanding. `FR-ADM-04`/`06`/`12`/`13` remain pending |
...
| **FR-DATA-01…11** (listing data, incl. `FR-DATA-06b/06c`, `FR-DATA-11b/11c`) | Data; V3–V5 | Data · **P1** | `OQ-5`; ~~`OQ-6`~~, ~~`OQ-7`~~, ~~`OQ-8b`~~ **Decided** | **Location fields Committed** (`OQ-6`): `FR-DATA-04` locality **required**, `FR-DATA-06` country **required**, `FR-DATA-05` administrative area **optional**, `FR-DATA-06b` postal code **optional**, `FR-DATA-06c` street address **not collected**. **Public projection Committed** (`OQ-7`): `FR-DATA-11`, `FR-DATA-11b` classification, `FR-DATA-11c` business-designated contact visibility. **Designation authority and defaults Committed** (ungated Product Owner policy clarification, `ADR-017` Q-4, issue #135): the business designates each contact value and a supplied postal code at submission, **default private**; administrators restrict only; new or replacement values default private without inheriting; postal code stays location data — `OP-3`/`S3`, `OP-6`/`S7`. **Contact minimum Committed** (`OQ-8b`): **`FR-DATA-08` now `Must`** — at least one **usable** phone, email, or website **before approval**, never at initial submission; location is not contact; **no offline-business exemption**. `FR-DATA-07` unchanged — each contact field stays individually optional. Rest Committed | pending |
...
| **FR-AUD-01…06** (status & auditability) | Data; A4, A5, A6 | Data / admin · **P1/P4** | `OQ-14`+`NOQ-8` (audit log); ~~`OQ-13`~~ **Decided** (rejected retention) | **AUD-05 Conditional** (`OQ-14`/`NOQ-8`); **AUD-06 Committed and raised to Must** (`OQ-13` — retain 90 days from rejection, then purge as a system obligation; measured from a **write-once rejection timestamp**, separate from last-updated, retention-only — ungated Product Owner policy clarification, issue #137); rest Committed | `FR-AUD-06` — **retention eligibility only, domain level, `P1` Slice E** (issue #149): the **boundary** is implemented and attacked as a **fixed elapsed duration** of exactly 90 × 24 hours — 7,776,000,000 ms — measured from each record's write-once rejection anchor, with a covered record **not** eligible before it and eligible **at** and after it, under **one uniform rule** for a rejected listing and a rejected revision (ungated Product Owner boundary clarification, issue #149, `docs/13`; `src/domain/listing/retention.ts`, `retention.test.ts`). Eligibility is a **query** — it is **derived, not stored** (`ADR-017`), permits a purge and performs none, and changes nothing about the record, which stays administrator-visible. **Nothing else of `FR-AUD-06` is implemented:** purge scheduling and execution, persistence, store-level atomicity and idempotence (`DI-3`, `DI-7`, `BI-7`, `C9`), backup expiration and restoration (`NFR-BACK-04`), application operations, authorization, audit emission (`OQ-14`/`NOQ-8`) and the physical representation (`DDM-9`) all remain **unimplemented**, so `FR-AUD-06`, `NFR-PRIV-05`, `S-11` and `RC-1` are **not** complete. `FR-AUD-01…05` remain pending |
...
is an invariant with no evidence.** The test column is **pending** except where a merged
increment names its attacking test. Elsewhere no test exists yet, and an empty cell here is
honest (`DG-4` must not be pre-answered with an aspirational test). The standard, when the
cell is filled, is *"we tried to break it and could not"* — proven **at the level where the
invariant is enforced** (the operation, not the UI — `BI-9`, `IP-5`), not merely "the test
passes."
...
|---|---|---|---|---|
| `DI-1` | Exactly one status at all times | `NFR-DATA-01`, `FR-AUD-01` | P1 | **domain level, `P1` Slice A** (issue #109, merged by PR #114): the governed status set is exactly the three values, a listing carries exactly one of them, and any value outside the set is refused as a governed `Result` error rather than accepted or thrown on (`src/domain/listing/status.ts`, `status.test.ts` — *listing status set (FR-AUD-01, DI-1)*). **Domain-level enforcement only** — that a **stored** record always carries exactly one status is a persistence obligation (`ADR-017`, `C9`), not implemented or proven here |
| `DI-2` | Status changes only via permitted transition | `NFR-DATA-02` | P1 | **domain level, `P1` Slice A** (issue #109, merged by PR #114), with the instant rule added by **Slice C** (issue #141, PR #142): the permitted ordered pairs are enumerated, every forbidden pair is refused, and a transition additionally requires a strictly later supplied instant (`src/domain/listing/status.ts`, `status.test.ts` — *permitted transitions (NFR-DATA-02, DI-2)*). **Partial** — the invariant's *"only through a defined administrator action"* half is **authorization** (`DI-4`, `BI-5`, `DG-3`) and is **not** implemented; no `OP-*` operation, screen or authenticated actor exists |
| `DI-3` | Every action completes fully or not at all (never partially public) | `NFR-DATA-03` | P1 | pending — **unimplemented at the level it is enforced.** Store-level atomicity is a persistence concern (`BI-7`, `C9`, `DDM-8`/`DDM-9` selected by `ADR-017`) and no persistence exists yet. Slice D expresses the **domain** half for one operation only — an approved revision's content application and proposal removal travel as **one** successful value, with no partially applied shape (`src/domain/listing/revision.ts`) — which is **not** a claim that `DI-3` is proven |
| `DI-4` | Administrative attributes settable only by system/admin | `FR-AUD-04`, `NFR-DATA-04` | P1 | pending — **unimplemented; blocked by `DG-3`.** Authorization, authenticated identity and the administrative surface are later-layer work (`BI-5`, `C8`, **P4 first**); nothing in `src/domain/` distinguishes a public from an administrative caller |
| `DI-5` | No non-approved record reachable through any public path | `FR-VIS-02`, `NFR-PRIV-03` | P1/P2 | **partial — domain level only, `P1` Slice A** (issue #109, PR #114), extended by **Slice B** (issue #139, PR #140): the logical public projection returns one generic unavailable outcome for a *pending*, *rejected*, absent or **unpublished** listing, disclosing nothing about which (`src/domain/listing/public-projection.ts`, `public-projection.test.ts` — *record-level exposure (FR-VIS-02, DI-5)*). **Outstanding:** the invariant is stated over **paths** — browse, search, direct reference by identity, and restored backups — and none of those paths exists. `BI-1`/`BI-4` operation-level enforcement, including the `BI-4` **equivalence test**, remains **P2** and is **its own issue** |
| `DI-6` | `submitted at` write-once; `last updated at` on every content **or status** change, each requiring a **strictly later** instant (issue #141); rejection timestamp write-once and present iff *rejected* (issue #137) | `FR-AUD-02/03`, `NFR-DATA-05`, `FR-AUD-06` | P1 | **domain level, `P1` Slice C** (issue #141, merged by PR #142): the domain increment is **on `main`**, and its attacking tests prove caller-supplied logical instants, write-once `submitted at`, a **strictly advancing** `last updated at`, a write-once rejection timestamp present **iff** *rejected*, timestamps **preserved** across publication-state changes, and **governed refusal** of malformed or chronologically impossible timestamp state (`src/domain/listing/instant.ts`, `timestamps.ts` and their tests). **Domain-level enforcement only** — none of the following is implemented or proven by these tests: `ADR-017` has **selected** the physical design, but its persistence implementation is **not part of Slice C**; retention execution, purge scheduling and execution, and restoration remain **Deferred** and outstanding (`docs/08`); clock sourcing, time-zone handling and serialization are **Excluded** from Slice C and none is selected here; revision timestamps and audit policy remain **undecided** |
| `DI-7` | Stored data reflects last successful action; no silent loss | `NFR-DATA-06` | P1 | pending — **unimplemented at the level it is enforced.** The invariant is about **stored** data; no store, repository or persistence mechanism exists (`C9`, `ADR-017`). The domain's immutable-value style and governed refusals are a precondition for it, not evidence of it |
| `DI-8` | Identity stable for the record's whole life | `docs/08` (`DDM-2` physical) | P1 | **domain level, `P1` Slice A** (issue #109, merged by PR #114): identity is a value of its own, compared by its supplied value rather than by object reference, and it is preserved unchanged across content edits, permitted status transitions and revision approval (`src/domain/listing/listing-id.ts`, `listing-id.test.ts` — *listing identity (DI-8)*, which attacks preservation across a content edit and across both status transitions; preservation across revision approval is re-attacked in `revision.test.ts`). **Domain-level enforcement only** — the physical representation is `DDM-2`, **selected** by `ADR-017` and **not implemented**; identity stability across a store, a restore or a process boundary is not proven here |
| `DI-9` | Category value always a member of the predefined set | `FR-DATA-10`, `FR-DATA-02` | P1 | pending — **unimplemented; shaped by `OQ-5`** (category model — cardinality and curation, **Unresolved**; it blocks no phase, and set membership is required regardless — `docs/13`, `S-3`/`DDM-3`). Validation currently requires a category to be present and usable (`src/domain/listing/validation.ts`), which is **not** membership of a predefined set; no predefined set exists |
| `DI-10` | Public read paths expose only the currently approved listing version; a pending revision is **not** returned, rendered, indexed, searched, or otherwise exposed through any public read path | `FR-ADM-10`, `FR-VIS-02`, `NFR-PRIV-01/03` | P1/P2 | **partial — domain level only.** The logical public projection is built from the listing's effective public version alone, and a pending revision supplied alongside it changes no successful projection and no unavailable outcome (`src/domain/listing/public-projection.ts`, `public-projection.test.ts` — *pending-revision invisibility (DI-10)*, introduced in **Slice A**, issue #109/PR #114, and preserved through **Slice B**'s publication gating, issue #139/PR #140). **Slice D** (issue #145, merged by PR #146) re-attacks the same boundary at the resolution operations: revision content reaches no successful projection and no unavailable outcome before, during or after approval or rejection (`src/domain/listing/revision.test.ts` — *revision content never reaches a public outcome (DI-10)*). **Outstanding:** `DI-10` is stated over **paths**, and no public read path, renderer, index or search exists — **P2** operation-level enforcement (`BI-4`, `C4`) is **not** complete |
| `DI-11` | An approved listing has no more than one **pending** revision at a time; it constrains the pending state, not revision history | `FR-ADM-10` | P1 | **domain level, `P1` Slices A and D.** **Slice A** (issue #109, PR #114) admits a proposal into the pending state and refuses a second while one is pending; **Slice D** (issue #145, merged by PR #146) adds the pending state's two exits — approval and rejection — and proves that resolving the sole pending revision **frees the admission slot** while a second pending revision stays refused (`src/domain/listing/revision.ts`, `revision.test.ts` — *the pending-revision constraint (DI-11)*, *resolving the sole pending revision (DI-11, FR-ADM-10)*). Each operation resolves the listing's **sole** pending revision named by this rule: **no logical revision identifier, ordering identity or positional identity is introduced**, and a reconstructed collection resolves exactly as the original does. History is **not** restricted — rejected and other retained revisions keep their place and value (`FR-AUD-06`). **Domain-level enforcement only** — cross-process and store-level concurrency (the per-listing lock and pending-uniqueness backstop `ADR-017` **selected** under `DDM-8`) is **not** implemented, and the physical revision representation is **not** built |
...
| [`ADR-006`](adr/ADR-006-listing-data-model-and-lifecycle-states.md) | Listing data model & lifecycle states | **Accepted** — 2026-08-07 (issue #61) | **Nothing** — ~~`OQ-6/7/8/8b/10/11/13`~~ all **Decided**, `DG-1` **Resolved** (`DD-1`). **In force**; work may rely on it. Discharges `DD-1` **logically only**; `DDM-8` and `DDM-9` ~~remain **unresolved**~~ are **resolved by `ADR-017`** (Accepted 2026-09-17) |
...
| [`ADR-012`](adr/ADR-012-test-tooling.md) | Testing strategy — **test-tooling portion** | **Accepted** — 2026-09-04 (issue #107); **in force**, and later implementation may rely on it. Published as `Proposed` 2026-09-03 (issue #105, PR #106). The decision is **Vitest** as the primary implementation-level test runner, beginning with **`P1` Slice A**, whose logical/domain foundation is its first consumer | **Nothing, for the tooling half** — `ADR-002` (**Accepted** 2026-08-25) fixes the stack it must serve. Testing **depth** remains **`DG-4`**, which is **`Unresolved`** and **not** resolved by selecting a runner: no coverage provider, coverage threshold or release-readiness criterion is decided (`NFR-MAINT-03`, `R-6`). **`DG-2` remains `Resolved`** (2026-08-27, issue #93) and is **not** reopened; `ADR-012` is **not** a `DG-2` constituent. **The runner is now installed and enabled** (issue #110): Vitest is a development dependency, one `test` script runs it once without watch mode, and one **tooling verification test** proves it executes. **No Vitest configuration file exists**, **no complementary test layer is selected**, **CI does not run tests**, and `P1` Slice A remains unimplemented and separately governed (issue #109); **Slice A decides no `DDM-*`**; and no persistence technology or provisioning decision arises |
| [`ADR-013`](adr/ADR-013-managed-postgresql-provider.md) | Named managed PostgreSQL provider (`DDM-1`) | **Accepted** — 2026-09-03 (issue #103); **in force**. Published as `Proposed` 2026-09-01 (issue #101, PR #102). The decision is **DigitalOcean Managed PostgreSQL** — the named provider [`ADR-003`](adr/ADR-003-data-store-product.md) deferred; **it discharges `DDM-1` only as to the named managed service and vendor** | **Nothing** — `ADR-003` (**Accepted**) fixed the engine and managed posture; [`ADR-010`](adr/ADR-010-backup-recovery-availability-posture.md) (**Accepted**) supplies the capability filter; `DG-2` is **`Resolved`** (2026-08-27, issue #93) **for its own existing reasons — this ADR is not a `DG-2` constituent and acceptance opens no gate**. **`DDM-1` is discharged only as to the named provider**; `DDM-2`–`DDM-10` are untouched. `ADR-010` is **not discharged**: its **independent off-provider copy**, **restore rehearsals** and **provider-capability validation** remain **outstanding**; **Render remains the application host only** (`ADR-005`); region, tier, sizing, PostgreSQL version, provisioning, ORM, driver, pooler, migrations, schema, indexes, authentication, secrets, backup mechanism and connectivity **all remain deferred**; **implementation remains unperformed, unauthorized and separately governed** |
| [`ADR-014`](adr/ADR-014-postgresql-data-access-approach.md) | `C9` PostgreSQL data-access approach — the ORM / query-builder / direct-driver question ruling **`R-A`** deferred | **Accepted** — 2026-09-11 (issue #117); **in force**, and later `C9` implementation may rely on it. Published as `Proposed` 2026-09-09 (issue #115, PR #116). The decision is a **typed SQL query-builder approach — Kysely** — as the `C9` PostgreSQL data-access approach, with explicit transaction control part of the selected approach; **the PostgreSQL driver/client is NOT selected** and remains an implementation-time deferred choice, as does the **connection pooler** | **Nothing** — `ADR-002` (**Accepted**) established ruling **`R-A`**; `ADR-003` and [`ADR-013`](adr/ADR-013-managed-postgresql-provider.md) (**Accepted**) fix the engine and the named provider; [`ADR-005`](adr/ADR-005-hosting-platform-and-runtime-model.md) (**Accepted**) fixes the long-running Node runtime; `ADR-006` (**Accepted**) the lifecycle the store must express; [`ADR-010`](adr/ADR-010-backup-recovery-availability-posture.md) (**Accepted**) the recovery-correctness criteria; `DG-2` is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** a constituent of it. **`DDM-10` is NOT resolved** — migration and schema-evolution tooling remains **separately governed** later work, **not created** by this ADR, and **no candidate forecloses it** (Prisma and Drizzle **narrow** it; Kysely and direct `pg` **leave it open**); **`DDM-2`–`DDM-9` remained unresolved at this ADR's acceptance** (`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` since discharged by `ADR-017`, 2026-09-17), and this ADR does **not** claim any candidate selection would resolve them; `ADR-010`'s **independent off-provider copy**, **restore rehearsals** and **provider-capability validation** remain **outstanding**; and no driver/client, physical schema, index, migration, pooler, pool size, PostgreSQL version, provisioning, credential, secret, database-test infrastructure, coverage, CI or `DG-4` decision is made. **Nothing is installed and no implementation exists** |
| [`ADR-015`](adr/ADR-015-migration-and-schema-evolution-tooling.md) | Migration and schema-evolution tooling (`DDM-10`) | **Accepted** — 2026-09-14 (issue **#121**); **in force**. Published as `Proposed` 2026-09-13 (issue #119, PR #120). The decision is **Kysely Migrator** — Kysely's built-in `Migrator` — as the migration and schema-evolution tooling approach; the **owner ruling of 2026-09-14 is recorded and authoritative**; `node-pg-migrate` is preserved as a serious alternative, stronger on migration-history portability, and rejection rationale is preserved for every category (`IR-6`) | **Nothing** — `DDM-10` carries **no blocker** in [`docs/08`](08-data-model.md); [`ADR-003`](adr/ADR-003-data-store-product.md), [`ADR-013`](adr/ADR-013-managed-postgresql-provider.md) and [`ADR-014`](adr/ADR-014-postgresql-data-access-approach.md) (all **Accepted**) supply the engine, the named provider and the `C9` access approach, and `ADR-014` classified Kysely as **`LEAVES OPEN`** for `DDM-10`; `DG-2` is **`Resolved`** (2026-08-27, issue #93) and this ADR is **not** a constituent of it. **`DDM-10` is RESOLVED** by this `Accepted` ADR through the selection of Kysely `Migrator`, and [`docs/08`](08-data-model.md)'s `DDM-10` row records it. **The PostgreSQL driver/client remains deferred and unselected** (neither `pg` nor Postgres.js nor any other), as do the **Kysely dialect**, the **application connection pool** and any **external pooler**. **`DDM-2`–`DDM-9` remained unresolved at this ADR's acceptance** (`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` since discharged by `ADR-017`, 2026-09-17), and **migration contents, migration authoring format beyond what the candidate makes inseparable, migration execution environment and timing, and rollback/down-migration policy all remain unresolved**; `ADR-010`'s **independent off-provider copy**, **restore rehearsals** and **provider-capability validation** remain **outstanding**, and `ADR-010` is expressly recorded as imposing **no** constraint on the candidate set. No physical schema, table, column, key, constraint, index, PostgreSQL version, provisioning, credential, secret, CI database infrastructure, coverage or `DG-4` decision is made. **Nothing is installed, no dependency is added, and no implementation exists or is authorized**; the register's decisions-in-force count was **ten** at this ADR's acceptance |
| [`ADR-016`](adr/ADR-016-postgresql-runtime-driver-and-kysely-dialect.md) | Runtime PostgreSQL driver/client and Kysely dialect for `C9` (non-DDM) | **Accepted** — 2026-09-15 (issue **#125**); **in force**. Published as `Proposed` 2026-09-14 (issue #123, PR #124). The decision is **`pg` (node-postgres) through Kysely core `PostgresDialect`** — the Product Owner's approval of Option A on 2026-09-15 from the governed comparison with Postgres.js through `kysely-postgres-js`, which is preserved as a legitimate alternative (`IR-6`) | **Nothing** — [`ADR-003`](adr/ADR-003-data-store-product.md), [`ADR-005`](adr/ADR-005-hosting-platform-and-runtime-model.md), [`ADR-013`](adr/ADR-013-managed-postgresql-provider.md), [`ADR-014`](adr/ADR-014-postgresql-data-access-approach.md) and [`ADR-015`](adr/ADR-015-migration-and-schema-evolution-tooling.md) (all **Accepted**) supply every constraint. **No numbered `DDM` owns or is resolved by this decision**; `DDM-2`–`DDM-9` remained unresolved at this ADR's acceptance (`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` since discharged by `ADR-017`, 2026-09-17) and `DDM-10` (resolved by `ADR-015`) is unaffected. **The runtime PostgreSQL driver/client and Kysely dialect are selected**, and ADR-016 selects nothing further: unselected are **application pool configuration** and any **external pooler**; no package version, TLS policy, secrets, provisioning, migration execution or rollback decision is made. **No package is installed and no implementation exists or is authorized**; the register's decisions-in-force count was **eleven** at this ADR's acceptance |
| [`ADR-017`](adr/ADR-017-physical-listing-data-design.md) | Physical listing data design (`DDM-2`, `DDM-6`, `DDM-8`, `DDM-9`) | **Accepted** — 2026-09-17 (issue **#137**); **in force**. Published as `Proposed` 2026-09-16 (issue **#131**). The Product Owner approved physical selections **PS-1 through PS-11** and their documented deferrals; the alternatives are preserved as considered alternatives; the five Product Owner questions it raised are **ruled** (Q-3 issue #133, Q-4 issue #135, Q-1/Q-2/Q-5 and atomic-path eligibility issue #137); coordinating `DDM-8` and `DDM-9` is a **design recommendation**, not a mandatory sequencing rule | **Nothing** — `DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` carry no unresolved blocker in [`docs/08`](08-data-model.md); it must conform to [`ADR-006`](adr/ADR-006-listing-data-model-and-lifecycle-states.md) and be consistent with [`ADR-014`](adr/ADR-014-postgresql-data-access-approach.md), [`ADR-015`](adr/ADR-015-migration-and-schema-evolution-tooling.md) and [`ADR-016`](adr/ADR-016-postgresql-runtime-driver-and-kysely-dialect.md) (all **Accepted**). **`DDM-2`, `DDM-6`, `DDM-8` and `DDM-9` are discharged** by this `Accepted` ADR, and [`docs/08`](08-data-model.md)'s `DDM` rows record it. **Excluded and untouched:** `DDM-3` (category representation — **category persistence requires `OQ-5`/`DDM-3` first**), `DDM-4`, `DDM-5`, `DDM-7`; `OQ-4`, `OQ-5`, `OQ-14`; seam `S-7`; migration execution and rollback policy; provisioning, pool, TLS, secrets and environment configuration; integration-test infrastructure and CI. **This ADR introduces no executable SQL, schema files, migrations or application code; this ADR installs nothing** (`pg` 8.23.0 and `kysely` 0.29.6 were already installed through issue #129, PR #130; `@types/pg` remains absent and deferred)**; and no implementation is authorized**; the register's decisions-in-force count is **twelve** |
...
~~`S-11` rejected retention & purge~~ — **resolved:** ~~`OQ-13`~~ **Decided** (90 days from rejection for both covered record types, then purge as a system obligation; representation `DDM-9` — selected by `ADR-017` (Accepted 2026-09-17); `ADR-006` (Accepted) settles the logical lifecycle only). **A cell that fills a seam with a guessed field
...
- **Committed by `OQ-10`:** `E7` listing revision, `OP-10` approve a pending revision, and the
  `S7` two-version view. **The revision *storage* mechanism is `DDM-8`, selected by `ADR-017` (Accepted 2026-09-17).**
- **Committed by `OQ-11`:** `OP-9` unpublish/republish, and the `S6` unpublish and republish
  ...
  publication state is `DDM-9`, selected by `ADR-017`** (Accepted 2026-09-17; `ADR-006`, Accepted,
  settles the logical concept), and the control's form is undrawn.
