...
| [`ADR-013`](adr/ADR-013-managed-postgresql-provider.md) | Named managed PostgreSQL provider (`DDM-1`) | **Proposed** — 2026-09-01 (issue #101); **not in force**. The proposed decision is **DigitalOcean Managed PostgreSQL** — the named provider [`ADR-003`](adr/ADR-003-data-store-product.md) deferred | **Nothing** — `ADR-003` (**Accepted**) fixed the engine and managed posture; [`ADR-010`](adr/ADR-010-backup-recovery-availability-posture.md) (**Accepted**) supplies the capability filter; `DG-2` is **`Resolved`** (2026-08-27, issue #93). **`DDM-1` is not discharged by a `Proposed` ADR**, and on acceptance only as to the named provider; `ADR-010`'s **independent off-provider copy**, restore rehearsals and provider-capability validation remain **outstanding**; **Render remains the application host only** (`ADR-005`); region, tier, sizing, provisioning, ORM, migrations, schema, authentication and connectivity **all remain deferred**; **implementation remains unauthorized** |
