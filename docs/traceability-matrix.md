...
| Technology stack | `DG-2` | **Hard blocker** | `P0b` and everything after | Unresolved — **`ADR-005` alone remains** (owner ruling 2026-08-26, issue #87) | ~~`ADR-002`–`ADR-005`~~ `ADR-002` (**Accepted**), `ADR-003` (**Accepted**), `ADR-010` (**Accepted**), **`ADR-005`**. **`ADR-004` is `DG-3`**, not a `DG-2` constituent |
...
| [`ADR-005`](adr/ADR-005-hosting-platform-and-runtime-model.md) | Hosting platform & runtime model | **Proposed** — 2026-08-26 (issue #89); **under review, not in force**. The proposal is **Render + a long-running Node.js web service**. **`DD-5` not discharged** | **Nothing** — ~~`DG-2` (`DD-5`)~~; `DD-5` is a stated **input** and `NOQ-2` is **Decided**. **`DG-2` membership stands**, and `ADR-005` is `DG-2`'s **final remaining substantive constituent** (owner ruling, issue #87); `DG-2` remains **Unresolved** |
