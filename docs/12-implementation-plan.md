...
| Revision **storage** (`DDM-8` — the mechanism, not the entity), duplicate representation, and the **representation** of retention and purge | **Open** — `DDM-8`, `OQ-12`, `DDM-9`. `E7` itself is **committed** by `OQ-10`; how it is persisted is not. **Retention and purge are committed as policy by `OQ-13`** (90 days from rejection, then purge); **how** they are represented is `DDM-9`/`ADR-006`. | `S-5`, `S-10`, `S-11` |
