...
| **`ADR-005`** hosting platform and runtime model | **Nothing** — `DD-5` is a stated **input** (`docs/07` records that both runtime shapes satisfy the architecture), and `NOQ-2` is **Decided**. **`Ready to write`** (`docs/adr/README.md`). **It is the final remaining substantive `DG-2` constituent** (owner ruling, issue #87), and **`DG-2` stays Unresolved until it is `Accepted`** |
...
| **Technology stack** | ~~Language, framework, store, hosting, auth, CI, test tooling?~~ **Language, framework, store, hosting?** — **`ADR-005` hosting and runtime model is all that remains** (owner ruling, issue #87); **auth is `DG-3`**, **CI is `P0b`**, and **test tooling does not block `DG-2`** | **`DG-2`** | **`P0b` and everything after it.** Only `ADR-001` (the architecture *shape*) is decidable today. |
