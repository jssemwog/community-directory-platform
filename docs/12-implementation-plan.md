...
| Seams left **unbuilt**, and recorded as unbuilt | `S-7`, `S-8`, `S-9`, `S-10`, `S-11` *(`S-5` is now **fully resolved** — `OQ-10` and `OQ-11` are both Decided)* | Audit, anti-spam, duplicate, and retention structures exist **only if** the corresponding question resolves to "yes." The unpublish/republish **capability** is approved (`OQ-11`), but its **representation** is still unbuilt (`ADR-006`, `DDM-9`). `IP-7`. |
