...
| **`DG-1`** — **data design** | **Hard blockers:** ~~**`OQ-6`**~~ **Decided** · ~~**`OQ-7`**~~ **Decided** · ~~**`OQ-8` / `OQ-8b`**~~ **Decided** (required fields and contact minimum) · ~~**`OQ-10`**~~ **Decided** (edit-after-approval) · ~~**`OQ-11`**~~ **Decided** (unpublish/republish) · ~~**`OQ-13`**~~ **Decided** (rejected-record retention and purge). **All six hard blockers are answered; `DG-1` is Resolved.**<br>**Shaping inputs:** **`OQ-4`** searchable fields and matching mode (`S-4`, `DDM-4`) · **`OQ-5`** category model — cardinality and curation (`S-3`, `DDM-3`) | **All of `P1`.** Transitively `P2`, `P3`, `P4`. | `docs/07` `DD-1` states the six hard blockers outright: they **must be answered before data design starts**. `OQ-6` retrofitted onto a populated directory means backfilling every record; `OQ-10` **added a revision entity (`E7`) and no lifecycle state** (`S-5`, resolved for `OQ-10`), decided before `P1` and at zero data volume; `OQ-7` is the control that `BI-6` and `NFR-PRIV-01` rest on — **until it is answered, the privacy commitment is an empty promise** (`docs/07` `R-2`); `OQ-8`/`OQ-8b` **filled the `S-1` obligation set** (`VR-S1`–`VR-S3`) **without adding a field and without selecting a schema or constraint mechanism**, so `P1` must still express them.<br>~~**Four of the six hard blockers are now Decided. `OQ-11` and `OQ-13` remain, so `DG-1` does not clear.**~~ — **`OQ-11` and `OQ-13` were both Decided 2026-08-04, so all six hard blockers are answered and `DG-1` is `Resolved`.**<br>**`OQ-4` and `OQ-5` shape rather than block:** each lands on a named data-model seam (`S-4`, `S-3`), so the *rule* can be built and the *representation* deferred — but a model built without considering them will be retrofitted, not merely configured. |
| **`DG-2`** — **technology** | **Hard blockers (store selection, `ADR-003`):** **`NOQ-2`** availability — **Decided** · **`NOQ-3`** backup / RPO / RTO — **Decided**. **Plus the technology stack itself** — ~~language, framework, store, hosting, authentication, CI, test tooling~~ ~~**language, framework, store, hosting** — **Unresolved**~~ — **all Decided**. **Three hard blockers: all three Decided — and `DG-2` itself is now `Resolved`** (2026-08-27, issue #93), through the separate governed synchronization an ADR could not perform. **Owner ruling 2026-08-26 (issue #87) — `DG-2`'s remaining closure scope is `ADR-005` alone.** `ADR-002`, `ADR-003` and **`ADR-005`** are **Accepted**, so ~~hosting and the runtime model (`ADR-005`, `DD-5`) are all that remain~~ — **`ADR-005` is `Accepted` 2026-08-27 (issue #91): Render, with the Next.js modular monolith deployed as a long-running Node.js web service, discharging `DD-5`. The final substantive `DG-2` architecture decision is complete**, and ~~only the separate governed `DG-2` → `Resolved` synchronization remains; until it lands, `DG-2` stays `Unresolved` and implementation stays unauthorized~~ — **that synchronization has landed (issue #93): `DG-2` is `Resolved` 2026-08-27, so `P0b` is eligible and `P1` is gate-clear. `DG-3` and `DG-4` still bind everything they govern.** **Authentication is `DG-3`** (`ADR-004`, `NOQ-9`); **`ADR-012` does not block `DG-2`** and testing **depth** is `DG-4`; **CI is `P0b` work this gate unblocks**, not a closure constituent; and **`DDM-1`'s deferred named managed provider does not block closure**.<br>**Shaping inputs (technology selection, `ADR-002`, `ADR-005`):** **`NOQ-1`** performance thresholds · **`NOQ-4`** expected first-release load | **The `P0` scaffold** and all of `P1`. | **The store cannot be chosen against `NOQ-2` / `NOQ-3`** — `docs/07` `DD-3` is explicit that a store **cannot be responsibly chosen against an unknown recovery point objective**, because the objective determines the store's required *capability*, not merely a schedule. **`NOQ-1` and `NOQ-4` do not block the choice; they inform it** (`docs/07` files both under *before technology selection*). They must be **considered and carried as a stated assumption** — `PA-1` is exactly that assumption — because everything here assumes "small," and if that is wrong, this is the first decision to revisit (`DD-12`). |
...
> `NOQ-5`**. This plan adds only the consequence: ~~**`DG-1` is now Resolved, but `DG-2` is
> not — so the only implementation work that can honestly proceed is still the
> stack-neutral half of Phase 0.**~~ **Both gates are now `Resolved`** — `DG-1` 2026-08-04,
> `DG-2` 2026-08-27 (issue #93) — so **`P0b` is eligible and `P1` is gate-clear.** Clearing a
> gate removes a blocker; it does not start the work, and `P1` still follows `P0b`.
...
**The tables below are not the ADR register.** `P0` delivers **the ADRs**, and workstream
`W1` owns them across `P0` and `P1`; each table lists the ADR work that half was planned
around, not every ADR the project records. **`ADR-006` — listing data model and lifecycle
states — is one the tables do not list.** It was governed under `P0`/`W1` through issue #61
and the ADR workflow, and was **Accepted 2026-08-07**. No phase row was ever assigned to it,
and none is assigned retrospectively here. Its authoritative status lives in the ADR register
(`docs/adr/README.md`).
...
### `P0b` — stack-dependent. ~~**Blocked by `DG-2`.**~~ **`DG-2` is `Resolved` (2026-08-27, issue #93); `P0b` is eligible to start.**

| Work | Blocked on |
|---|---|
| **`ADR-002`** language and application framework | ~~`DG-2`~~ — **Nothing.** **Accepted** — 2026-08-25 (issue #85, PR #86); the decision is **TypeScript and Next.js**, and **it discharges `DD-2`**. **`ADR-002` is a `DG-2` constituent, not blocked by `DG-2`** — a gate's constituent cannot be blocked by the gate it constitutes; `docs/07` hard requirements 1, 2 and 6 are stated **inputs**, not blockers. ~~**`ADR-005` is the remaining `DG-2` architecture decision** (owner ruling, issue #87), so **`DG-2` stays Unresolved and `P0b` stays blocked**~~ — **`ADR-005` is `Accepted` 2026-08-27 (issue #91) and `DG-2` is `Resolved`** (2026-08-27, issue #93), so **`P0b` is eligible to start**. Eligible is not started: it still has to be commissioned |
| **`ADR-003`** data-store product | **`DG-2`** — technology stack; `docs/07` `DD-3`. ~~`NOQ-2`, `NOQ-3`~~ are **both Decided**, and **Accepted `ADR-010`** supplies the recovery-capability filter `DD-3` required, so **`ADR-003` is `Accepted`** — 2026-08-23 (issue #81); **in force**. The decision is **PostgreSQL under a managed operating posture**, and **it discharges `DD-3`**. **The named provider is deferred, so `DDM-1` is not fully discharged; no schema or index is decided; and ~~`DG-2` remains Unresolved — `P0b` and `P1` stay blocked~~ **`DG-2` is `Resolved`** (2026-08-27, issue #93), so `P0b` is eligible and `P1` is gate-clear.** |
| **`ADR-004`** administrator authentication mechanism | **`DG-3`** — `NOQ-9`, `DD-4`. *(The boundary is settled; the mechanism is not.)* **`ADR-004` is a `DG-3` decision and does not block `DG-2` closure** (owner ruling, issue #87); it appears in this table because the ADR was planned here, not because `DG-2` holds it |
| **`ADR-005`** hosting platform and runtime model | ~~`DG-2`~~ — **Nothing.** **Accepted** — 2026-08-27 (issue #91, PR #92); published as `Proposed` 2026-08-26 (issue #89, PR #90). The decision is **Render, hosting the Next.js modular monolith as a long-running Node.js web service**, and **it discharges `DD-5`**. ~~**`Ready to write`**~~ — that pointer is superseded by the register's `Accepted` status. **It was the final remaining substantive `DG-2` constituent**, so **`DG-2`'s substantive decisions are now complete** — but **an ADR opens no gate**, so ~~**`DG-2` stays `Unresolved`** pending a **separate governed `Resolved` synchronization**, and **`P0b` stays blocked**~~ — **that separate governed synchronization is this one: `DG-2` is `Resolved`** (2026-08-27, issue #93), so **`P0b` is eligible to start** |
| Repository scaffold, project structure along the `C1`–`C12` seams, local development setup | ~~`ADR-002`~~ **Accepted** — 2026-08-25 (issue #85); **TypeScript and Next.js**, discharging `DD-2`. ~~**`P0b` remains blocked by `DG-2`**, which is still **Unresolved**~~ — **`DG-2` is `Resolved`** (2026-08-27, issue #93), so **this work is eligible to start**. Eligible is not started: it still has to be commissioned |
| Continuous-integration pipeline; automated checks | ~~`ADR-002`~~ **Accepted**; the testing tool still follows from it via `ADR-012`. ~~**CI's own gate classification remains unresolved**~~ — **settled by owner ruling 2026-08-26 (issue #87): CI is `P0b` / derivative implementation-enablement work that `DG-2`'s closure unblocks, not a `DG-2` closure constituent**, which is how `CONTRIBUTING.md` already treats it. **Selecting a CI system is therefore not a prerequisite for closing `DG-2`**, and **no CI system is selected**. ~~**`DG-2` remains Unresolved**~~ — **`DG-2` is `Resolved`** (2026-08-27, issue #93), so this work is **eligible**; the CI system itself is still an open `P0b` choice |
...
> `DG-2`.~~ **The product owner has now answered both** — `DG-1` **Resolved** 2026-08-04,
> `DG-2` **Resolved** 2026-08-27 — so `P0b` is eligible and `P1` is gate-clear. Teams under delivery pressure will be tempted to "start on the model and adjust
...
other phase stands on. ~~**Hard-blocked by `DG-1` and `DG-2`.**~~ **Both gates are `Resolved`** — `DG-1` 2026-08-04, `DG-2` 2026-08-27 (issue #93) — so **`P1` is gate-clear**; it still follows `P0b` and has not been commissioned.
...
| Identity, timestamps, integrity rules | `DI-6`–`DI-9` | Identity is stable and content-independent (`DDM-2` is a physical choice, selected by `ADR-017`, Accepted 2026-09-17). |
| The **public/private field boundary as a projection**, enforced server-side | `S-2`, `DI-5` | **The mechanism (`DDM-6`) is selected by `ADR-017`** (Accepted 2026-09-17); the *obligation* is fixed separately. Whether withheld fields live on the same record or a separate structure is not decided here. |
| Category set representation | `S-3` | Follows `OQ-5`. Configuration or data — `DDM-3` decides *after* the product answer. |
| The **revision lifecycle** — entity `E7`, `DI-10`, `DI-11` | `FR-ADM-10`, `FR-ADM-10b`, `S-5` (resolved for `OQ-10`) | **Committed by `OQ-10`.** A pending revision is never publicly visible; the approved listing stays public at its last approved version; at most one pending revision per listing. **The storage mechanism is `DDM-8`, selected by `ADR-017` (Accepted 2026-09-17).** |
| Seams left **unbuilt**, and recorded as unbuilt | `S-7`, `S-8`, `S-9`, `S-10` *(`S-5` and `S-11` are now **fully resolved** — `OQ-10`, `OQ-11`, and `OQ-13` are Decided)* | Audit, anti-spam, and duplicate structures exist **only if** the corresponding question resolves to "yes." **Rejected-record retention and purge are now approved and must be built** (`FR-AUD-06`: retain 90 days from rejection, then purge as a system obligation) — though its **representation** stays unbuilt (`DDM-9`, `ADR-006`). The unpublish/republish **capability** is approved (`OQ-11`), but its **representation** is still unbuilt (`ADR-006`; `DDM-9` selected by `ADR-017`, Accepted 2026-09-17). `IP-7`. |
...
| `OP-9` (unpublish/republish) | **Committed** — `OQ-11` Decided 2026-08-04. Built in **`P4`** with the other administrative operations | `S-5` — **resolved**. Carries both directions; enforces the required reason and explicit confirmation. **Representation is not settled** (`ADR-006`, `DDM-9`), so it cannot be built ahead of `P1`. |
...
| ~~`OQ-11`~~ **Decided 2026-08-04** | **Removal / unpublish?** | **`DG-1`** | **Answered:** reversible unpublish and explicit republish, modelled as **publication state separate from listing status** — the `P1` status model is **unchanged** (`FR-AUD-01` intact, no new transition). **`OP-9` is committed** and built in `P4`. Permanent deletion excluded; representation deferred to `ADR-006`/`DDM-9`, since selected by `ADR-017` (Accepted 2026-09-17). |
| `OQ-12` | Duplicate resolution? | — | Nothing structural. An administrator procedure; an exploratory charter. |
| ~~`OQ-13`~~ **Decided 2026-08-04** | **Rejected-submission retention?** | **`DG-1`** | **Answered:** rejected initial submissions **and** rejected approved-listing revisions are retained **90 days from rejection**, administrator-visible only, **terminal**, then purged. `P1` gains the retained state and the **purge obligation** (`S-11` resolved); purge is a **system obligation**, not an administrator action. Retention *and* period resolved together (`NFR-PRIV-05`, now `Must`). Representation: `DDM-9`, selected by `ADR-017` (Accepted 2026-09-17). |
...
| **Technology stack** | ~~Language, framework, store, hosting, auth, CI, test tooling?~~ ~~**Language, framework, store, hosting?**~~ — **all Decided**: ~~`ADR-005` hosting and runtime model is all that remains~~ — **`ADR-005` is `Accepted` 2026-08-27 (issue #91), discharging `DD-5`**; ~~only the formal `DG-2` → `Resolved` synchronization remains, so `DG-2` stays `Unresolved`~~ — **that synchronization has landed: `DG-2` is `Resolved` 2026-08-27 (issue #93)** (owner ruling, issue #87); **auth is `DG-3`**, **CI is `P0b`**, and **test tooling does not block `DG-2`** | ~~**`DG-2`**~~ — **Resolved** | ~~**`P0b` and everything after it.** Only `ADR-001` (the architecture *shape*) is decidable today.~~ — **discharged: `P0b` is eligible and `P1` is gate-clear.** `P3`, `P4` and `P2`'s *verifiability* remain blocked by `DG-3`; the release decision remains `DG-4`. |
...
| `OP-9` (unpublish/republish) | **Committed** (`OQ-11` Decided). Built in `P4`. | `docs/09`, `S-5` — **resolved** |
...
| Revision **storage** (`DDM-8` — the mechanism, not the entity), duplicate representation, and the **representation** of retention and purge | **Open** — `OQ-12`. ~~`DDM-8`, `DDM-9`~~ are **selected by `ADR-017`** (Accepted 2026-09-17). `E7` itself is **committed** by `OQ-10`, and its persistence is selected by `ADR-017`. **Retention and purge are committed as policy by `OQ-13`** (90 days from rejection, then purge); **how** they are represented is `DDM-9`, selected by `ADR-017`. | `S-5`, `S-10`, `S-11` |
...
because **until they clear, no application code can honestly be written.**~~ **Both were taken
to the product owner and both have cleared** — `DG-1` 2026-08-04, `DG-2` 2026-08-27 — so
`P0b` is eligible and `P1` is gate-clear. Then build the
