...
| **Administrator** | **Yes** | View the pending queue; open, edit, approve, and reject; edit approved listings; unpublish and republish approved listings (OQ-11, Decided — reason and confirmation required) | Bypass validation; act without the action being attributable | FR-ADM-01..13, FR-AUTH-02, NFR-SEC-01 |
...
- **Now owns the publication-state change, and one remaining deferred seam.**
  **OQ-11 is Decided (2026-08-04):** C6 performs the authorized unpublish and
  republish of an approved listing (FR-ADM-12) — a **publication-state** change,
  **not** a listing-status transition, so C6 remains the sole status writer and
  FR-AUD-01's three values and NFR-DATA-02's permitted edges are untouched. C6
  enforces the required reason and the explicit confirmation, and enforces that
  approving a revision while a listing is unpublished does **not** republish it.
  **OQ-13 is Decided (2026-08-04):** a rejected record — initial submission or
  approved-listing revision alike — is retained **90 days from the rejection**,
  administrator-visible only, and is then purged. Rejection is **terminal**: C6
  admits no transition out of *rejected*, so FR-AUD-01 and NFR-DATA-02 are
  untouched. Purge is a **system obligation**, not an administrator action, and it
  never alters an approved listing. **All six DG-1 hard blockers are now
  answered**, and the lifecycle seams C6 owned are closed.
...
sequenceDiagram
    ...
    alt approve
        ...
        C6->>C9: status := REJECTED (retained 90 days, then purged - OQ-13)
...
- **No persistence mechanism is selected.** How the effective public version is
  carried is DDM-8, selected by ADR-017 (Accepted 2026-09-17).
...
| **Administrator to moderation** | Approve, reject, edit, unpublish/republish (OQ-11, Decided) | C6 refuses to act without an authorized identity. Authorization is not delegated to the interface layer; C3 being hidden is not the control. | FR-MOD-02, NFR-SEC-01 |
...
| Rejected records *(retention — OQ-13, **Decided**)* | A listing record with rejected status for **90 days from rejection**, then purged | FR-AUD-06 and NFR-PRIV-05, both now **Must** — see below |
...
**A privacy tension, now resolved.** FR-AUD-06 said retain rejected submissions for
audit; NFR-PRIV-05 said non-public data must not be kept indefinitely by default.
These pulled in opposite directions, and **OQ-13 resolved both at once**
(2026-08-04): retention *with a defined period and purpose* — **90 days from the
rejection**, then purge — covering rejected initial submissions and rejected
approved-listing revisions under one uniform rule. Both requirements are now
...
retention and purge **is DDM-9, selected by ADR-017 (Accepted 2026-09-17)** — see ADR-006,
ADR-017 and, in docs/08, DDM-9.
...
| **Publishing a field the submitter never meant to be public** | C4 projects to the **public field set** before data leaves the domain. **Which fields those are is settled by OQ-7 — Decided 2026-07-31 — so this control now has content; its enforcement mechanism, DDM-6, is selected by ADR-017 (Accepted 2026-09-17).** | FR-DATA-11, NFR-PRIV-01/02 |
...
there was its enforcement mechanism, **DDM-6, since selected by ADR-017 (Accepted 2026-09-17)**. OQ-9 still means nothing yet stops
...
|---|---|---|---|
| **DD-1** | **Detailed data design** — fields, types, keys, indexes, and how the category set is represented | It depends on unresolved product decisions; guessing now would silently resolve them | **No longer blocked by any DG-1 product question.** **OQ-6**, **OQ-7**, **OQ-10**, **OQ-8/8b**, **OQ-11**, and **OQ-13** are now **Decided** — OQ-13 fixed rejected-record retention at 90 days from rejection with a committed purge obligation, selecting **no representation**, so the data design still has to express it — OQ-11 authorised reversible unpublish/republish as **publication state**, adding **no listing status** and selecting **no representation**, so the data design had to express it (ADR-006, DDM-9 — since selected by ADR-017, Accepted 2026-09-17) — OQ-10 committed entity E7 and the revision lifecycle without adding a listing status; OQ-8/8b settled the field obligations and the contact minimum **without adding, removing, or renaming any field**, and deliberately selected **no schema type, database constraint, or validation mechanism** — so the data design still has to express them. **The remaining zero must be answered before data design starts.** |
| **DD-2** | Language and application framework | ~~Technology selection; needs the criteria above applied deliberately~~ — **Decided.** | Must satisfy hard requirements 1, 2, and 6. **DD-2 is discharged by Accepted ADR-002 (2026-08-25): TypeScript and Next.js.** The compensating obligations O-1–O-12 recorded there are binding. **No ORM or data-access technology, hosting platform, authentication mechanism, test tooling, or CI is decided**, and ~~DG-2 remains Unresolved~~ — **DG-2 is Resolved (2026-08-27, issue #93)** |
| **DD-3** | Data-store product | ~~Same. The *storage properties* are fixed; the product is not~~ — **Decided.** | Must satisfy hard requirements 3, 4, and 5. ~~**Cannot be responsibly chosen until NOQ-2 and NOQ-3** (availability, RPO/RTO) are answered~~ — **both Decided 2026-07-30** (NOQ-3 amended by ruling R1, 2026-08-18), so that precondition is satisfied. **DD-3 is discharged by Accepted ADR-003 (2026-08-23): PostgreSQL under a managed operating posture.** The **named provider remains deferred** (DDM-1), and no schema or index is decided |
...
| **DD-5** | Hosting platform and runtime model (long-running instance versus serverless runtime) | ~~Both satisfy the architecture; statelessness keeps both open~~ — **Decided.** | NFR-OPS-04. **Does not reopen the rejected decomposition of Option D.** **DD-5 is discharged by Accepted ADR-005 (2026-08-27): Render, with the Next.js modular monolith deployed as a long-running Node.js web service.** The compensating obligations O-1–O-9 recorded there are binding. Both runtime shapes did satisfy the architecture; the **long-running** shape was selected, and the container and serverless shapes remain **historical alternatives**, not current architecture. **No named PostgreSQL provider, Render service tier, ORM or data-access technology, authentication mechanism, test tooling, CI, container technology, or observability product is decided**, and ~~DG-2 remains Unresolved~~ — **DG-2 is Resolved (2026-08-27, issue #93)** |
...
| **DD-9** | Backup frequency, recovery point objective, recovery time objective | **No longer blocked by NOQ-3** — **Decided 2026-07-30**: daily backups, RPO up to 24 hours, RTO one business day, restore tested before launch and at least quarterly, ~~at least one independent off-provider copy refreshed weekly~~ **at least one independent off-provider copy no more than 24 hours behind the live data (amended R1, 2026-08-18 — the 24-hour RPO covers provider loss)**. **DD-9 is not thereby closed** — these fix the required backup *capability*, not merely a schedule; the mechanism that delivers it is still to be chosen | **NOQ-3 — Decided.** Feeds DD-3 |
...
| ~~**R-9**~~ **Closed 2026-08-04** | ~~The OQ-13 versus NFR-PRIV-05 tension resolves by drift~~ — rejected records retained "for audit" and then simply kept forever | ~~An indefinite store of non-public personal data that nobody decided to keep~~ | **Closed** | **OQ-13 resolved retention, period, and purpose together — 90 days from rejection — and committed the purge capability as a system obligation, which is what the drift risk required.** Residual: the obligation must actually be built; someone must own running it |
...
| **FR-AUD-01..06** (auditability) | C6, C9; **C10** *(conditional)* | F3 | FR-AUD-01..04 are committed. FR-AUD-05 depends on **OQ-14 / DD-7**; **FR-AUD-06 is settled by OQ-13** — retain 90 days from rejection, then purge, as a C6/C9 obligation; its representation is DD-9 |
...
| **Data integrity** (DATA-01..06) | A single transactional store; C6 as sole status writer; atomic writes; system-set administrative fields | **Lifecycle complete** — OQ-10, OQ-11, and OQ-13 are all Decided and **none added a listing status**; the revision lifecycle, the publication-state change, and the rejected-record end state are defined. Residual risk is **R-3b** |
...
| ~~**OQ-6**~~ **Decided 2026-07-31** | Location granularity (city, plus region, plus country) | **Answered: locality and country required; administrative area and postal code optional; no street address.** Locality — the town, city, village, municipality, or comparable named place — and country are **required on every listing**; country is drawn where practical from a **standardised list**, is **multi-country capable from launch**, and is never silently defaulted. Administrative area and postal code are **optional**, the postal code held as **text** in international formats and never used to infer a residential address. **No precise business or residential street address is collected in the MVP.** The FR-DATA-04..06 field set and the filter shape follow; the backfill risk did not materialise because the answer came before any record existed. Normalisation remains **DDM-5, open** | ~~Before data design~~ — **answered before DD-1** |
| ~~**OQ-7**~~ **Decided 2026-07-31** | Which fields are public versus withheld | **Answered: an explicit public projection, not the whole record.** Public: business name, category, short description; locality, country, and administrative area where provided; postal code **only** where it was provided and designated for public display; and each contact method — phone, email, website — the business designated public. Everything else is administrator-visible, audit-only, or outside MVP collection scope. The C4 projection that enforces NFR-PRIV-01/02 now has content rather than being an empty promise (**R-2**). The enforcement mechanism ~~remains **DDM-6, open**~~ is **DDM-6, selected by ADR-017 (Accepted 2026-09-17)** | ~~Before data design~~ — **answered before DD-1** |
...
| ~~**OQ-10**~~ **Decided 2026-08-02** | Edit after approval: publish immediately, or require secondary review? | **Answered: secondary review.** A pending revision (E7) is stored separately from the effective public version and is never public (DI-10); the approved listing stays public at its last approved version; at most one pending revision per listing (DI-11). **No lifecycle state was added.** FR-ADM-10b permits a safeguarded atomic administrator operation (**R-3b**). Storage mechanism ~~remains **DDM-8, open**~~ is **DDM-8, selected by ADR-017 (Accepted 2026-09-17)** | ~~Before data design~~ — **answered before DD-1** |
| ~~**OQ-11**~~ **Decided 2026-08-04** | Can an administrator unpublish or remove an approved listing? | **Answered: yes — reversible unpublish and explicit republish.** Modelled as **publication state, separate from listing status**, so **no fourth status** and **no new status transition**: FR-AUD-01 and NFR-DATA-02 are unchanged. A current reason and explicit confirmation are required; the listing leaves every public read path and direct links get the generic unavailable result; a pending revision stays pending and approving it does not republish. **Permanent deletion excluded from the MVP.** Representation ~~remains **ADR-006 / DDM-9, open**~~ is **DDM-9, selected by ADR-017 (Accepted 2026-09-17)**, conforming to ADR-006 | ~~Before data design~~ — **answered before DD-1** |
| ~~**OQ-13**~~ **Decided 2026-08-04** | Retention of rejected submissions | **Answered: retain 90 days from rejection, then purge.** Covers rejected initial submissions **and** rejected approved-listing revisions under one uniform rule; administrator-visible only; **terminal** — no transition out of *rejected*, so the C6 state machine is unchanged. **A purge capability is committed** as a **system obligation**, resolving R-9's drift risk. FR-AUD-06 and NFR-PRIV-05 are both now **Must** and **neither was amended**. Representation ~~remains **ADR-006 / DDM-9, open**~~ is **DDM-9, selected by ADR-017 (Accepted 2026-09-17)**, conforming to ADR-006 | ~~Before data design~~ — **answered before DD-1** |
...
| ~~**NOQ-2**~~ **Decided 2026-07-30** | Availability target | **Answered: 99% over a rolling monthly window, excluding announced maintenance.** The public read path takes priority over administrative tools. Recorded in NFR-REL-01/02/05. At 99% the deployment shape this document contemplates is one instance rather than a redundant one, and statelessness keeps redundancy an **operational** change rather than a redesign. **NOQ-2 no longer blocks DD-8; the hosting product is not selected** — that remains **ADR-005**, under DG-2 | ~~Before technology selection~~ — **answered before technology selection** |
| ~~**NOQ-3**~~ **Decided 2026-07-30** | Backup frequency, recovery point objective, recovery time objective | **Answered: daily backups; RPO up to 24 hours; RTO one business day.** A restore is **tested before launch and at least quarterly**; **at least one independent copy off the primary provider**, ~~refreshed at least weekly~~ **no more than 24 hours behind the live data**; and **equivalent confidentiality, access control, and protection for every copy**. Recorded in NFR-BACK-01–NFR-BACK-06. **Amended R1, 2026-08-18:** the 24-hour recovery point objective applies to **every in-scope recovery scenario, including complete loss of — or loss of access to — the primary provider**, so the weekly freshness guarantee is superseded; **no mechanism is selected**. **NOQ-3 no longer blocks DD-9**, and the DD-3 store-selection precondition it gated is satisfied — **the store product itself is still not selected**; that remains **ADR-003**, under DG-2 | ~~Before store selection~~ — **answered before store selection** |
...
| **ADR-002** | Application language and framework | Hard requirements 1, 2, 6; DD-2. **Accepted 2026-08-25 (issue #85); in force.** The decision is **TypeScript and Next.js**. **It discharges DD-2.** **No ORM/data-access technology, hosting platform, authentication mechanism, test tooling, CI, or separable frontend technology is decided**; ~~ADR-005 remains open; and DG-2 remains Unresolved~~ — **ADR-005 is Accepted and DG-2 is Resolved (2026-08-27, issue #93)** |
| **ADR-003** | Data-store product | Hard requirements 3, 4, 5; DD-3. **No longer blocked** — NOQ-2 and NOQ-3 are both **Decided 2026-07-30** (NOQ-3 amended by ruling R1, 2026-08-18), and **Accepted ADR-010** supplies the recovery-capability filter DD-3 required. **Accepted 2026-08-23 (issue #81); in force.** The decision is **PostgreSQL under a managed operating posture**. **It discharges DD-3.** **The named provider is deferred, so DDM-1 is not fully discharged; no schema or index is decided; ADR-010's provider-capability validation remains outstanding**; and ~~DG-2 remains Unresolved~~ — **DG-2 is Resolved (2026-08-27, issue #93)** |
| **ADR-004** | Administrator authentication mechanism | DD-4, NOQ-9 |
| **ADR-005** | Hosting platform and runtime model | DD-5. Must not reopen the rejected decomposition of Option D. **Accepted 2026-08-27 (issue #91); in force.** The decision is **Render, hosting the Next.js modular monolith as a long-running Node.js web service**. **It discharges DD-5.** **No named PostgreSQL provider, Render service tier, ORM/data-access technology, authentication mechanism, test tooling, CI, container technology, or observability product is decided; the rejected decomposition of Option D is not reopened**; and ~~DG-2 remains Unresolved~~ — **DG-2 is Resolved (2026-08-27, issue #93)** — this ADR completed the gate's final substantive decision but opened no gate; the gate was closed by its own governed synchronization |
| **ADR-006** | Listing data model and lifecycle states | DD-1. **No longer blocked by a DG-1 product question** — OQ-6, OQ-7, OQ-8/8b, OQ-10, OQ-11, OQ-13 all Decided. **Accepted 2026-08-07 (issue #61); in force.** It discharges DD-1 **logically only** — the physical representation remains DDM-1–DDM-10, and in particular ~~**DDM-8 and DDM-9, both still unresolved**~~ **DDM-8 and DDM-9, since selected by ADR-017 (Accepted 2026-09-17)** |
...
side.** **ADR-006 is now Accepted** (2026-08-07) and **in force**; DG-2 still governs the
technology they depend on, and remains Unresolved.
