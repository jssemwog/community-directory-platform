...
|---|---|---|---|
| `DDM-1` | **The store product.** No database engine, service or vendor is selected. | `docs/07` `DD-3`. A store cannot be responsibly chosen against an unknown recovery point objective — and ~~`NOQ-3`~~ is now **Decided**, so that precondition is satisfied. **What still holds it is the unresolved `DG-2` technology-stack decision**, which `ADR-003` (**`Proposed`** — drafted 2026-08-23, issue #81; **not `Accepted`** and **not in force**) settles. **`DDM-1` is not discharged by a `Proposed` ADR**, and the owner's provider-deferral ruling leaves the **service and vendor** outstanding in any case. A satisfied precondition is not authorization. | **`DG-2`** — technology stack (`ADR-003`); ~~`NOQ-3`~~ **Decided** |
...
plainly.** `ADR-006` (Accepted 2026-08-07, issue #61) settles the **logical** listing model and its
