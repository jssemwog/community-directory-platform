...
|---|---|---|---|
| `DDM-1` | **The store product.** No database engine, service or vendor is selected. | `docs/07` `DD-3`. A store cannot be responsibly chosen against an unknown recovery point objective — and ~~`NOQ-3`~~ is now **Decided**, so that precondition is satisfied. **What still holds it is the unresolved `DG-2` technology-stack decision**, which `ADR-003` (**Ready to write** — owner ruling 2026-08-19, issue #75; **not drafted, not `Proposed`, not `Accepted`, not begun**) settles. A satisfied precondition is not authorization. | **`DG-2`** — technology stack (`ADR-003`); ~~`NOQ-3`~~ **Decided** |
